...
class StaffListScreen extends StatefulWidget {
  final String gymId;
  const StaffListScreen({super.key, required this.gymId});
  ...
  void _confirmDelete(String docId) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Delete Staff', style: GoogleFonts.inter(color: Colors.white, fontWeight: FontWeight.w700)),
        content: Text('Remove this staff member?', style: GoogleFonts.inter(color: AppColors.textSecondary, height: 1.5)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Cancel', style: GoogleFonts.inter(color: AppColors.textMuted))),
          TextButton(
            onPressed: () { Navigator.pop(ctx); FirebaseFirestore.instance.collection('staff').doc(docId).delete(); },
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text('Staff', style: GoogleFonts.inter(fontSize: 22, fontWeight: FontWeight.w800, color: Colors.white)),
                ],
              ),
            ),
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance.collection('staff').where('gymId', isEqualTo: widget.gymId).snapshots(),
                builder: (ctx, snap) {
                  ...
                  if (docs.isEmpty) {
                    return Center(
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.badge_outlined, color: AppColors.textMuted, size: 48),
                          const SizedBox(height: 12),
                          Text(_query.isEmpty ? 'No staff added yet.' : 'No results found.',
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final id = docs[i].id;
                      final name = d['name'] as String? ?? 'Staff';
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Text('₹$salary / month', style: GoogleFonts.inter(fontSize: 11, color: const Color(0xFF10B981), fontWeight: FontWeight.w700)),
                                  ),
                                ],
                              ),
                            ),
